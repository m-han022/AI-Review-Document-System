        ...
        <div className="viewer-pane-left">
          <div className="viewer-toolbar-v3">
            <div className="viewer-pagination-v3">
              <button type="button" className="page-nav-btn" onClick={() => { const idx = slideReviewItems.findIndex((s) => s.id === activeSlide.id); if (idx > 0) setSelectedSlideId(slideReviewItems[idx - 1].id); }} disabled={slideReviewItems.findIndex((s) => s.id === activeSlide.id) === 0}>
                <ChevronLeftIcon size="sm" />
              </button>
              <span className="page-indicator-v3">Slide {activeSlide.slide_number} / {slideReviewItems.length}</span>
              <button type="button" className="page-nav-btn" onClick={() => { const idx = slideReviewItems.findIndex((s) => s.id === activeSlide.id); if (idx < slideReviewItems.length - 1) setSelectedSlideId(slideReviewItems[idx + 1].id); }} disabled={slideReviewItems.findIndex((s) => s.id === activeSlide.id) === slideReviewItems.length - 1}>
                <ChevronRightIcon size="sm" />
              </button>
            </div>
            <a href={getSubmissionFileUrl(projectId, "inline")} target="_blank" rel="noreferrer" className="external-link-v3"><MaximizeIcon size="sm" /> {t("project.openFull")}</a>
          </div>
          <div className="document-stage-v3" style={{ flex: 1, minHeight: 0 }}>
            <div className="slide-preview-frame">
              {gradingDetail?.document_version?.file_path?.toLowerCase().endsWith(".pdf") ? (
                <iframe src={getSubmissionFileUrl(projectId, "inline") + `#page=${activeSlide.slide_number}`} className="slide-iframe-v3" title={`Slide ${activeSlide.slide_number}`} />
              ) : (
                <div className="preview-placeholder-v3">
                  ...
                  <Button type="button" variant="secondary" size="sm" onClick={(e) => { e.preventDefault(); e.stopPropagation(); window.open(getSubmissionFileUrl(projectId, "attachment"), "_blank"); }} style={{ marginTop: "12px" }}>
              ...
              <div className="analysis-main-col">
                  ...
                  <section className="analysis-section-v3">
                    ...
                    <div className="issue-list-v3">
                      {activeSlide.issues.map((issue: string, idx: number) => (
                        <div key={idx} className="issue-card-v3"><AlertTriangleIcon size="sm" /><span>{issue}</span></div>
                      ))}
                    </div>
                  </section>
                )}
                {activeSlide.suggestions && (
                  <section className="analysis-section-v3">
                    ...
                    <div className="analysis-card-v3 is-suggestion"><SparkIcon size="sm" /><div>{activeSlide.suggestions}</div></div>
                  </section>
                )}
              </div>
